...
dsh -i $identFile centos@$hostAddr 'sudo ./initpkg.sh'
dsh -i $identFile centos@$hostAddr 'sudo ./run.sh'

dcp -i $identFile -r centos@$hostAddr`:/tmp/sysbench-out "~/Desktop/$($btd_Defaults.EC2.InstanceType)-crdb"

touch "~/Desktop/$($btd_Defaults.EC2.InstanceType)-crdb/$(Get-Date -UFormat '%Y%m%d%H%M%Z')"
